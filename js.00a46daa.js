...
var facets = ['Frame Rate', 'Aspect Ratio', 'Subtitles', 'Status', 'File Format'];
var facetData = [{
  name: 'Frame Rate',
...
var xIcon = "<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 32 32\" preserveAspectRatio=\"none\">\n<polygon points=\"25.192 5.393 16 14.586 6.808 5.393 5.393 6.808 14.586 16 5.393 25.192 6.808 26.607 16 17.414 25.192 26.607 26.607 25.192 17.414 16 26.607 6.808 25.192 5.393\"></polygon>\n</svg>";

var onSearchInput = function onSearchInput(e, tag, tagName) {
  emptyDropdown();
  var val;
  var properties;

  if (tag) {
    properties = facetData.filter(function (f) {
  ...
  var matches = properties.filter(function (facet) {
    var facetSubstr = facet.toLowerCase().substr(0, val.length);
...
var onRowSelect = function onRowSelect(e) {
  input.value = '';
  ...
  tag.addEventListener('input', function (e) {
    if (e.target.value.length < length) {
      ...
      var widthNR = tag.value.length;
      tag.style.width = widthNR + 'ch';
    }

    onSearchInput(e, tag, tag.value.split(': ')[0]); // let offset = tag.offsetWidth - 20;
  ...
  }, tag, tag.value.split(': ')[0]); // let offset = tag.offsetWidth - 20;
  // dropdown.style.left = offset + 'px ';
};

var emptyDropdown = function emptyDropdown() {
  dropdown.innerHTML = '';
};

var onRowSelectInTag = function onRowSelectInTag(tag, text) {
...
var appendDropdownRow = function appendDropdownRow(text, tag) {
  var row = document.createElement('button');
  row.className = 'dropdown-row';
  if (tag) row.addEventListener('click', function () {
    return onRowSelectInTag(tag, text);
...
document.onkeydown = function (e) {
  if (e.key === 'Backspace' && input.value.length === 0 && input === document.activeElement && input.previousElementSibling) {
